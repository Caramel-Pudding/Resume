...
function extractBetween(text: string, start: string, end?: string) {
  ...
  return (e === -1 ? text.slice(s + start.length) : text.slice(s + start.length, e)).trim();
}
...
function resolvePdfPath(input?: string) {
  ...
  const fromPublic = candidate.startsWith("public/") ? candidate : path.join("public", candidate);
  return path.resolve(process.cwd(), fromPublic);
}

export async function getResumeFromPdf(pdfPath?: string): Promise<ResumeDraft> {
  ...
  try {
    ...
    throw new Error(
      ...
      `Place your file in /public and call getResumeFromPdf("Profile.pdf").`
    );
  }

  const buf = await fs.readFile(filePath);
  if (!buf?.length) throw new Error(`PDF at ${filePath} is empty or unreadable`);

  const { text } = await pdf(buf);

  const links = Array.from(new Set(Array.from(text.matchAll(/https?:\/\/\\S+/g)).map(m => m[0])));
  const summary = extractBetween(text, "Summary", "Experience") ?? extractBetween(text, "About", "Experience");
  const experienceRaw = extractBetween(text, "Experience", "Education");
  const educationRaw = extractBetween(text, "Education");
  const firstLines = text.split(/\\r?\\n/).map(l => l.trim()).filter(Boolean);
  const name = firstLines.find(l => /^[A-Z][a-z]+(?: [A-Z][a-z]+)+$/.test(l));
